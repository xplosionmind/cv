...
#let showrules(uservars, doc) = {
	...
	show heading.where(
		...
		#set align(left)
		#set text(font: uservars.headingfont, size: 1em, weight: "extrabold")
		#upper(it.body)
		#v(-0.75em) #line(length: 100%, stroke: .7pt + gradient.linear(..color.map.plasma,)) // Draw a line
	]
	...
	show heading.where(
		...
		#set text(font: uservars.headingfont, size: 1.5em, weight: "black")
		#upper(it.body)
		#v(2pt)
	]
...
#let addresstext(info, uservars) = {
	if uservars.showAddress {
		block(width: 100%)[
			#info.personal.location.city, #info.personal.location.country #info.personal.location.postalCode
			#v(-4pt)
		]
	} else {none}
}
...
#let contacttext(info, uservars) = block(width: 100%)[
	...
	#pad(x: 0em)[
		#profiles.join([#sym.space.en #sym.circle.filled.tiny #sym.space.en])
	]
]
...
#let endnote = {
	place(
		bottom + right,
		block[
			#set text(size: 5pt, fill: rgb("#0004"))
			#set par(leading: 2pt)
			This CV was last updated on #datetime.today().display("[year]-[month]-[day]"), using #link("https://typst.app")[Typst]. Its #link("https://codeberg.org/tommi/cv")[source code] is licensed under #link("https://www.apache.org/licenses/LICENSE-2.0")[Apache License 2.0]. The content of the CV is licensed under #link("https://creativecommons.org/publicdomain/zero/1.0/")[CC0].
		]
	)
}
